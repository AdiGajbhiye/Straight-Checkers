    ...
    */
    ...
    static int ApplyMove(int[][] board,int srtI,int srtJ,int endI,int endJ)
    {
        ...
        if (result != illegalMove)
        {
            ...
            // check for new king
            if ( board[endI][endJ] == Checker.REDNORMAL && endJ == 7)
            {
                board[endI][endJ] = Checker.REDKING;
            }

            else if ( board[endI][endJ] == Checker.YELLOWNORMAL && endJ == 0)
            {
                board[endI][endJ] = Checker.YELLOWKING;
            }
    ...
    */
    static int isMoveLegal(int[][] board,int srtI,int srtJ,int endI,int endJ,int turn)
    {
        ...
        if ( Math.abs(srtI - endI) == 1 )
        {
            ...
            switch (piece)
            {
                case Checker.REDNORMAL:
                case Checker.REDKING:
                    for (int i=0;i<8;i++)
                    {
                        for (int j=0;j<8;j++)
                        {
                    ...
                    break;
                case Checker.YELLOWNORMAL:
                case Checker.YELLOWKING:
                    for (int i=0;i<8;i++)
                    {
                        for (int j=0;j<8;j++)
                        {
                            if ((board[i][j] == Checker.YELLOWNORMAL ||
                                    board[i][j] == Checker.YELLOWKING)
                                    && canCapture(board,i,j))
                                return illegalMove;
                        }
        ...
        else if ( Math.abs(srtI - endI) == 2 )
        {
            ...
            if (turn == Checker.REDNORMAL)         //when you try to cut a piece it should be a peice of opposite side.
            //otherwise it is an illegal move.
            {
                if (!(cap_piece == Checker.YELLOWNORMAL ||
                        cap_piece == Checker.YELLOWKING))
                    return illegalMove;
            }
            else if (!(cap_piece == Checker.REDNORMAL ||       //same for yellowers
                    cap_piece == Checker.REDKING))
                return illegalMove;
    ...
    static boolean canCapture(int[][] board, int i, int j)
    {
        switch (board[i][j])
        {
            case Checker.REDNORMAL:
                ...
                if (i+2<8 && j+2<8)
                    if ( (board[i+1][j+1] == Checker.YELLOWNORMAL ||
                            board[i+1][j+1] == Checker.YELLOWKING)
                            &&
            ...
            case Checker.YELLOWNORMAL:
                if (i+2<8 && j-2>-1)
                    if ( (board[i+1][j-1] == Checker.REDNORMAL ||
                            board[i+1][j-1] == Checker.REDKING)
                            &&
                            board[i+2][j-2] == Checker.EMPTY)
                        return true;
                if (i-2>-1 && j-2>-1)
                    if ( (board[i-1][j-1] == Checker.REDNORMAL ||
                        ...
                        return true;
                break;
            case Checker.REDKING:   //should check all four directions   
                if (i+2<8)
                {
                    if (j+2<8)
                        if ( (board[i+1][j+1] == Checker.YELLOWNORMAL ||
    ...
    static int colour(int piece)
    {
        switch (piece)
        {
            case Checker.REDNORMAL:
            case Checker.REDKING:
                return Checker.REDNORMAL;
            case Checker.YELLOWNORMAL:
            case Checker.YELLOWKING:
                return Checker.YELLOWNORMAL;
        }
