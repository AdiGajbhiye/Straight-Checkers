...
public class GameEngine {
     ...
     * <p/>
     * For normal checkers;
     * Eval=(weightOfChecker + PositionWeight*(position)^2)*numberOfChecker + randomWeight;
     * <p/>
     * For kings;
    ...
    static int MinMax(int[][] board, int depth, int maxDepth, int[] move, int turn, int[] counter, int redBest, int yellowBest) {
        ...
        Vector movesList = new Vector();  //vector of 4x1 arrays
        ...
        if (depth == maxDepth) {
            ...
            movesList = Checker.generateMoves(board, turn);
            ...
            switch (movesList.size()) {
                ...
                case 1:
                    if (depth == 0) {
                        ...
                        System.out.println("depth == 0: RETURN CONTROL " + Checker.printboard(board));
                        System.out.println("RETURNED FORCED MOVE " + Checker.printMove(bestMove, turn));
                        return 0;
                    } else {
                    ...
                    break;
                case 2: {
                    //randomly pick a move if there is only 1 piece remaining
                    if (Checker.pieceCount(board, turn) == 1) {
                        Random random = new Random();
                        int pick = random.nextInt(1) + 1;
                        move = (int[]) movesList.elementAt(pick);
                        System.arraycopy(move, 0, bestMove, 0, 4);
                        System.out.println("case 2 : RETURN CONTROL " + Checker.printboard(board));
                        return 0;
                    }
                }
            }
            ...
            for (int i = 0; i < movesList.size(); i++) {
                ...
                Checker.moveComputer(newBoard, (int[]) movesList.elementAt(i)); /*returns new_board (changing start and end coodinates
